...
for(let item of area){
    // item == area배열에 담긴 요소를 순차적으로 하나씩 꺼내 저장하는 변수
...
for(let i=0; i<boxColor.length; i++){
    ...
    boxColor[i].addEventListener("change", function(){
        ...
        // boxColor[i].style.color = this.value; // 글자 색 변경
        this.style.color = this.value; // 글자 색 변경
...
document.getElementById("clearBtn").addEventListener("click", function(){
    ...
    for(let item of boxColor){ // .box-color값 삭제
        item.value="";
